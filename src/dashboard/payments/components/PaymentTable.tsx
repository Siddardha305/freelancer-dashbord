...
export function PaymentTable({ initialPayments = [] }: { initialPayments?: Payment[] }) {
      ...
      <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-50/50 animate-none">
                <th className="px-4 sm:px-6 py-5 text-[10px] font-bold text-slate-500 uppercase tracking-widest">Invoice</th>
                <th className="px-4 sm:px-6 py-5 text-[10px] font-bold text-slate-500 uppercase tracking-widest">Client</th>
                <th className="px-4 sm:px-6 py-5 text-[10px] font-bold text-slate-500 uppercase tracking-widest">Amount</th>
                <th className="px-4 sm:px-6 py-5 text-[10px] font-bold text-slate-500 uppercase tracking-widest">Due Date</th>
                <th className="px-4 sm:px-6 py-5 text-[10px] font-bold text-slate-500 uppercase tracking-widest">Status Action</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {payments.map((payment) => (
                <tr key={payment.id} className="hover:bg-slate-50/50 transition-colors group">
                  ...
                  <td className="px-4 sm:px-6 py-6">
                    <span className="text-base font-bold text-slate-900 tracking-tight">{formatCurrency(payment.amount)}</span>
                  </td>
                  <td className="px-4 sm:px-6 py-6">
