...
import { KpiCard } from "@/dashboard/overview/components/KpiCard";
      ...
      <header className="flex h-20 shrink-0 items-center justify-between gap-x-4 border-b border-slate-200 bg-white px-8">
        ...
        <div className="flex items-center gap-4">
          <div className="flex flex-col items-end mr-2">
            ...
            <span className="text-sm font-semibold text-slate-900">{currentUser?.name || 'Admin Manager'}</span>
          </div>
          <div className="h-10 w-10 rounded-xl bg-slate-50 flex items-center justify-center border border-slate-200 font-bold text-slate-700">
      ...
      <main className="flex-1 overflow-y-auto p-8 lg:p-12">
        <div className="mx-auto max-w-7xl space-y-10">
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-5">
            ...
            <KpiCard title="Total Revenue" value={formatCurrency(thisMonthRevenue)} icon={DollarSign} trend="This month" />
            <KpiCard title="Pending" value={formatCurrency(pendingPayments)} icon={Clock} trend="Awaiting" alert={pendingPayments > 0} />
                       ...
                       <div className="w-full flex flex-col justify-end items-center h-full">
                         ...
                         >
                           <div className="absolute -top-12 left-1/2 -translate-x-1/2 bg-slate-900 text-white text-[10px] font-bold px-3 py-2 rounded-xl opacity-0 group-hover/bar:opacity-100 transition-all duration-200 whitespace-nowrap shadow-lg">
                             {formatCurrency((thisMonthRevenue / 6 * (i+1)).toFixed(0))}
                           </div>
                         </div>
                       </div>
