...
import { adminResetPasswordAction } from '@/admin/actions/admin-actions';
import { ShieldCheck, KeyRound, Loader2, CheckCircle2, ArrowLeft, AlertCircle } from 'lucide-react';

const initialState = { message: '' };

function AdminResetPasswordForm({ token }: { token: string }) {
  const [state, formAction, isPending] = useActionState(adminResetPasswordAction, initialState);
  ...
  if (state?.message === 'success') {
    return (
      <div className="text-center space-y-5 py-4">
        ...
        <div>
          <h2 className="text-xl font-extrabold text-slate-900 dark:text-slate-50">Password Reset!</h2>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-extrabold tracking-tight text-slate-900 dark:text-slate-100 mb-1">Set New Password</h1>
          <p className="text-xs font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-widest">Admin Password Reset</p>
        </div>
      ...
      <form action={formAction} className="space-y-5">
        ...
        <div className="space-y-2">
          <label htmlFor="password" className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 block">
            New Password
          </label>
        ...
        <div className="space-y-2">
          <label htmlFor="confirmPassword" className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 block">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2rem] shadow-xl shadow-slate-100/50 dark:shadow-none p-8 sm:p-10 transition-colors duration-300">
          {!token ? (
            <div className="text-center space-y-4 py-4">
              <div className="mx-auto h-14 w-14 rounded-2xl bg-rose-50 dark:bg-rose-950/30 border border-rose-100 dark:border-rose-900/30 flex items-center justify-center">
                <AlertCircle className="h-7 w-7 text-rose-500" />
              </div>
              <h2 className="text-xl font-extrabold text-slate-900 dark:text-slate-50">Invalid Reset Link</h2>
              <p className="text-sm font-semibold text-slate-500 dark:text-slate-400">
                This reset link is missing or invalid. Please request a new one.
              </p>
              <Link
                href="/admin/forgot-password"
                className="inline-flex items-center gap-2 px-6 py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-sm rounded-2xl transition-all"
              >
                Request New Link
              </Link>
            </div>
